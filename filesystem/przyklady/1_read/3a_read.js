...
let buffer = new Buffer.alloc(1024); // rozmiar bufora: 1kB
/* UWAGA
    ...
    Bufory w Node (JavaScript) nie są w żaden sposób związane z buforowaniem danych.
    Buforowanie danych to całkiem inny mechanizm.
    Bufory (buffers) w Node (JS) są wykorzystywane w Node do przetwarzania danych
    binarnych (binary data).

    Standardowo, Node (JavaScript) umie "współpracować" (przetwarzać) dane UNICODE.
...
/* UWAGA
    ...
    Obrazowo, obiekt klasy Buffer odnosi się do określonej lokalizacji (obszaru)
    w pamięci operacyjnej, podobnie jak tablice zawierające liczby całkowite.
    ...
    dane dowolnego typu (np. liczby, znaki), natomiast bufor odnosi się wyłącznie do
    danych binarnych.
    Ponadto, tablicę można modyfikować, bufory - nie. Rozmiar bufora jest stały i nie można
    go zmienić.
 */

// Określenie nazwy pliku (ze ścieżką):
let fileName = __dirname + '/files/dane.txt';
...
fs.open(fileName,               // nazwa pliku ze ścieżką
        ...
        (err, fd) => {          // funkcja callback (err reprezentuje ewentualny błąd,
                                ...
                                /* UWAGA
                                    Deskryptor pliku do identyfikator pliku
